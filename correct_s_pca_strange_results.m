...
function S_new = correct_s_pca_strange_results(S_old, pca_level, pca_image, coeffs, mean, m, channels)
    neighbourhood = 5;
...
%     S_new(:,:,2)=suby;
    ...
    function [subx, suby] = correct_one_pixel(x, y)
        ...
        neighbours = neighbours-mean;
        %neighbours = neighbours - pca_image(sub2ind([m,m],S_old(x,y,1),S_old(x,y,2)));%subtract self
%original, on pixels        [~,i]=min(sum(sum(abs(pca_level-repmat(reshape(double(neighbours),[1 neighbourhood^2-1 channels]),[m^2,1,1])),3),2));
        neighbours = neighbours * coeffs;
        [~,i]=min(sum(sum(abs(pca_level-repmat(reshape(double(neighbours),[1 10 channels]),[m^2,1,1])),3),2));
