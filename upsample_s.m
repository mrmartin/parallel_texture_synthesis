...
function S_new = upsample_s (S_old, m, l)
    ...
    S_new(2:2:end,1:2:end,1)=S_old(:,:,1);%one down
    S_new(2:2:end,1:2:end,2)=S_old(:,:,2)+m/2^l;
    S_new(1:2:end,2:2:end,1)=S_old(:,:,1)+m/2^l;%one right
    S_new(1:2:end,2:2:end,2)=S_old(:,:,2);%one right
